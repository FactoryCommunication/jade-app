...
export default function ProjectForm({ initial = {}, onSubmit, onCancel, loading, isAdmin }) {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: t }, { data: u }, { data: az }, { data: pe }] = await Promise.all([
  ...
  function toggleAzienda(az) {
    ...
    if (ids.includes(az.id)) {
      const newIds = ids.filter((i) => i !== az.id);
      const newNomi = nomi.filter((n) => n !== az.nome);
      setForm({ ...form, aziende_ids: newIds, aziende_nomi: newNomi, client: newNomi[0] || "", referente_id: "none", referente_nome: "" });
    } else {
      const newIds = [...ids, az.id];
      const newNomi = [...nomi, az.nome];
  ...
  async function handleCreateAzienda() {
    ...
    const { data: created } = await supabase.from("crm_aziende").insert({ nome: newAziendaNome.trim() }).select().single();
    const { data: updated } = await supabase.from("crm_aziende").select("*").order("nome", { ascending: true });
    setAziende(updated || []);
    if (created) {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...
      referente_id: form.referente_id === "none" ? null : form.referente_id,
      team_name: team?.name || "",
      ...
      budget_euro: form.budget_euro ? Number(form.budget_euro) : null,
    });
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="relative" ref={aziendaDropRef}>
              <button type="button" onClick={() => setAziendaDropOpen((v) => !v)}
                className="flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border border-input bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-ring">
                <span className="truncate text-left">
                  {(form.aziende_ids || []).length === 0 ? "Seleziona aziende..." : (form.aziende_nomi || []).join(", ")}
                </span>
              ...
              {aziendaDropOpen && (
                <div className="absolute z-50 mt-1 w-full rounded-md border border-border bg-white shadow-md max-h-56 overflow-y-auto">
                  {aziende.length === 0 && <p className="text-xs text-muted-foreground px-3 py-2">Nessuna azienda nel CRM.</p>}
                  {aziende.map((az) => {
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Select value={form.team_id} onValueChange={(v) => setForm({ ...form, team_id: v })}>
              <SelectTrigger className="bg-white"><SelectValue placeholder="Seleziona team" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Nessuno</SelectItem>
          ...
          <div className="space-y-2">
            ...
            <Select value={form.manager_id} onValueChange={(v) => setForm({ ...form, manager_id: v })}>
              <SelectTrigger className="bg-white"><SelectValue placeholder="Seleziona responsabile" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Nessuno</SelectItem>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <input type="color" value={colorValue} onChange={(e) => setForm({ ...form, color: e.target.value })}
                className="h-9 w-12 rounded-md border border-input cursor-pointer p-0.5 shrink-0" />
              <Input value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value })}
                placeholder="#6366f1" className="font-mono text-sm bg-white" maxLength={7} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input type="number" value={form.budget_hours} onChange={(e) => setForm({ ...form, budget_hours: e.target.value })} className="bg-white" />
          </div>
        </div>
        ...
        {isAdmin && (
          <div className="space-y-2">
            ...
            <Input type="number" value={form.budget_euro} onChange={(e) => setForm({ ...form, budget_euro: e.target.value })} placeholder="Es. 5000" className="bg-white" />
          </div>
        )}

        <div className="flex gap-3 pt-2">
          <Button type="submit" disabled={loading || (form.aziende_ids || []).length === 0}>
          ...
          <div className="space-y-3">
            <div className="space-y-2">
              ...
              <Input autoFocus value={newAziendaNome} onChange={(e) => setNewAziendaNome(e.target.value)}
                placeholder="Es. Azienda Rossi Srl" className="bg-white"
                onKeyDown={(e) => e.key === "Enter" && handleCreateAzienda()} />
            </div>
